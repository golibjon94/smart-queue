...
## Theme switcher (same as dashboard)
Add a floating **theme button** (palette icon) at the top-right of the login page
that opens the same 5-preset swatch popover as the dashboard
(**petrol / slate / navy / plum / light** — values in `tokens.css`). Selecting one
sets `data-theme` on the login root; all colours flip via variables, the neural
canvas re-inits, and the right-panel background follows `--brand-base` per preset.
The AI-spectrum, form card, and inputs are all token-based so they adapt
automatically. Persist to `localStorage` (`sq_theme`).
